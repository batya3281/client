...
import {
    ...
    TableRow,
    Button,
    Menu, // Import Menu from Material-UI
    MenuItem, // Import MenuItem from Material-UI
    Fade,
    Grow,
    Card,
    Tooltip,
...
export const PropertiesForSale = () => {
    ...
    const [contextMenu, setContextMenu] = useState({
        ...
        x: 0,
        y: 0,
        propertyId: null, // Store the ID of the property the menu is for
    });
    ...
    const handleContextMenu = (event, propertyId) => {
        ...
        setContextMenu({
            show: true,
            x: event.clientX, // Use clientX/Y for positioning relative to the viewport
            y: event.clientY,
            propertyId: propertyId, // Store the property ID
        });
    };

    const handleCloseContextMenu = () => {
        setContextMenu({ ...contextMenu, show: false, propertyId: null });
    };
    ...
    const update = () => {
        // Find the property to edit
        const propertyToEdit = propertiesForSale.find(p => p.propertyId === contextMenu.propertyId);
        if (propertyToEdit) {
            handleOpenEditDialog(propertyToEdit);
        }
        handleCloseContextMenu();
    ...
    const remove = async () => {
        dispatch(deletePropertyForSaleThunk({ propertyCode: contextMenu.propertyId, price: price }));
        handleOpenPriceDialog();
        dispatch(editMonthPriceN());
        handleCloseContextMenu();
    };
            ...
            <Box sx={{ position: 'relative' }}>
                                ...
                                <Table stickyHeader aria-label="properties for sale table">
                                    ...
                                    <TableBody>
                                        {propertiesForSale.map(property => (
                                            <StyledTableRow
                                                key={property.propertyId}
                                                selected={property.propertyId === contextMenu.propertyId}
                                                onContextMenu={(e) => handleContextMenu(e, property.propertyId)}
                                            >
                                                <StyledTableCell>
                                                    {property.propertyArea} m²
                ...
                >
                    ...
                    <DialogActions sx={{ px: 3, pb: 3 }}>
                        ...
                        <StyledButton onClick={ok}>{isEditing ? 'Save Changes' : 'Add Property'} {/* Dynamic button text */}</StyledButton>
                    </DialogActions>
                </Dialog>
